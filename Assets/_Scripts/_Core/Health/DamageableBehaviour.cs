...
namespace Core.Health
{
	...
	public class DamageableBehaviour : MonoBehaviour
    {
		...
		/// </summary>
		public Damageable configuration = new Damageable();
        ...
        public virtual void TakeCure(float damageValue, Vector3 damagePoint, IAlignmentProvider alignment)
        {
            HealthChangeInfo info;
            configuration.TakeCure(damageValue, alignment, out info);
            var damageInfo = new HitInfo(info, damagePoint);
            if (hit != null)
            {
                hit(damageInfo);
